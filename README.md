...
> ⚠️ **ВАЖНО:** База данных (`db/custom.db`) НЕ попадает в Git! После клонирования репозитория необходимо выполнить эти команды:

```bash
# Создание базы данных и применение схемы
bun run db:push

# ⭐ Заполнение базы начальными данными (39 компаний) - ОБЯЗАТЕЛЬНО!
bun run db:seed
...
**Без выполнения `bun run db:seed` сайт будет показывать пустые категории!**
...
## 🔬 Исследование и добавление новых компаний

### Процесс добавления компаний через нейросеть

Проект включает систему для полуавтоматического добавления новых компаний:
...
Используйте промпт из `prisma/PROMPT_RESEARCH.md` с любой нейросетью:

```bash
# Пример запроса к нейросети:
# 1. Скопируйте промпт из prisma/PROMPT_RESEARCH.md
# 2. Добавьте название и сайт компании
# 3. Получите JSON-ответ
```

### Шаг 2: Сохранение данных

Полученный JSON сохраните в файл:

```bash
# Сохраните JSON от нейросети в:
prisma/companies-to-add.json
```

### Шаг 3: Импорт в базу

```bash
# Импорт компаний из JSON в базу данных
bun run db:import

# Результат:
# ✅ Компания А → LLM (leader)
# ✅ Компания Б → Аудио (active)
# ⏭️  Компания В: уже существует (дубликат)
```

### Шаг 4: Сохранение на GitHub

```bash
# Экспорт данных в seed-файл
bun run db:export

# Отправка на GitHub
git add .
git commit -m "Добавлены новые компании"
git push origin main3
```

### Файлы для работы с компаниями

| Файл | Описание |
|------|----------|
| `prisma/PROMPT_RESEARCH.md` | Промпт для исследования компаний |
| `prisma/companies-to-add.json` | Шаблон для новых компаний |
| `prisma/import-companies.ts` | Скрипт импорта |

### Категории компаний

| Ключ | Название | Описание |
|------|----------|----------|
| `llm` | LLM (Чат-боты) | Языковые модели, RAG, чат-боты |
| `audio` | Аудио-аналитика | ASR, TTS, речевая аналитика |
| `video` | Видео-аналитика | CV, распознавание лиц, OCR |
| `dl` | Deep Learning | ML-платформы, MLOps, GPU |
| `gen` | Генеративный ИИ | Генерация изображений, видео, кода |
...
│   ├── export-to-seed.ts      # Экспорт данных в seed
│   ├── import-companies.ts    # Импорт компаний из JSON
│   ├── companies-to-add.json  # Шаблон для новых компаний
│   └── PROMPT_RESEARCH.md     # Промпт для исследования
├── src/
│   ├── app/
│   │   ├── api/               # API routes
...
| `bun run db:import` | Импорт компаний из JSON (после исследования) |
